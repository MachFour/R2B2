...
% MODEL SUMMARY: Each time the tempo and beat location needs to be estimated, we
% use the Viterbi algorithm to choose the most probable tempo period and beat
% location, according to the following model:

% for the kth set of feature frames (one for each of the n features), we have a
% state vector: X[k] = (T[k], B[k], M[k], W_i[k]). W_i are a set of n weights, one
% for each feature.
...
% audio) closest to the time at which the current feature frame ends (but not
% after)

% M[k] is an indicator variable which represents the rhythmic meter that the
% current slice of audio is in.  I'm not sure whether to make this for simple or
% compound time, or duple or triple time.  This may be used to determine the shape
% of the beat alignment function, or to look at harmonics in the autocorrelation
...
% Transition probabilities essentially maintain continuity between estimates, and
% are all reasonably intuitive. Progression of tempo, meter and weights are assumed
% independent, and next beat location is dependent only on the previous tempo as
% well as its own previous value.  Each probability is more or less a Gaussian
...
% The rest of this description focuses on the observation probabilities, which
% encode musical knowledge.  For the latter, we define F1, F2, F3, ... to be each
% of the n features, and write
% P(Observations | State) := P(F1, F2, F3, ... | T[n]
% 	= t, B[n] = b, M[n] = m, W_i[n] = w_i)                                       (1)

% We assume that, given the state, the observed features are independent (since
% they are meant to measure different aspects of the audio).  So then, we can split
% up the joint probability into a product for each feature F_i

% = (product over i of) P(F_i | T[n] = t, B[n] = b, M[n] = m, W_i[n] = w_i))     (2)

% Now the probability of observing the feature frame, given the tempo, beat
% location, and meter, is assumed to be proportional to the strength of the
% autocorrelation function at lag t, multiplied by a sequence of impulses spaced t
% seconds a part and shifted so that the latest one is at b seconds, multiplied by
% some metric of musical measure, for example the energy of the harmonics of the
% autocorrelation at harmonics 2 and 4 vs 3 and 6 (this comes from Davies and
% Plumbley, "Context-Dependent Beat tracking of Musical Audio") in equation form,
% this gives (up to a normalising constant for each term)

% = (product over i of) ACF_i(t)*BAF_i(t, b)*metric_measure_i(t, m)              (3)

% where ACF_i is the autocorrelation of feature i's frame, BAF is the 'beat
% alignment function' i.e. the inner product of the t-spaced impulses with feature
% i and metric_measure_i is some metric on the feature used to establish the meter

% the last problem is to integrate the weights. If we simply multiply each term by
% a weight, it doesn't really make sense. What's the probability of a observing
% feature given its weight?  I took a liberal view, and used the weight as a
% measure of 'how much discriminatory power' each feature has. If the weight is
% low, then we shouldn't be 'listening' to that feature so much if it's high, that
% feature has 'an important measure' of the observation probability.  One way to
% disregard one particular feature's term was to make the probability artificially
% higher, so a mismatch between the model and that feature's data isn't penalised
% so much.  Therefore, what could be done is to raise the previous probabilities in
% (3) to the power of the weight.  When the weight is 0, the probability term
% becomes 1, thus effectively removing it from the product.  When the weight is 1,
% then the probability is unchanged. % This gives,
...
% make the search tractable is to pick the peaks of the autocorrelation function
% for each feature - as is currently done - and evaluate state candidates only at
% these likely tempo values. If S tempo values are allowed as candidates at each
% iteration, and W is the number of weight values allowed, then the number of
% calculations is of the order of S*S*W*n, where n is the number of features.  Does
% restricting S to maybe 16 mean that the use of the Viterbi Algorithm is
% unwarranted?


% prior probability distribution on tempos lognormal distribution, with parameters
% taken from Klapuri's paper (Equation 22).  He takes it from Parncutt (1994) "A
% perceptual model of pulse salience"
...
methods (Static)
	...
	function p = tempo_transition_prob(old_tempo, new_tempo)
		if ~isempty(find(old_tempo <= 0, 1)) || ~isempty(find(new_tempo <= 0, 1))
		...
		%sigma = 0.2;
		%mu = log(old_tempo)*exp(sigma^2);

		%p = musical_model.lognormal_density(new_tempo, mu, sigma); %.* ...
			%musical_model.tempo_prior_prob(new_tempo);

		% try something simpler

		% tempo percentage change
		p = 1./(1+abs(new_tempo - old_tempo)./new_tempo).^4;
	end
	...
	function p = beat_location_transition_prob(old_state, new_state)
		...
		if tempo_percent_change < 2/100
			...
			tempo_period = new_state.tempo_period;
			sigma = tempo_period/16;

			expected_alignment = mod(old_alignment - hop_size, -new_tempo);
