...
function clusters = mean_shift_cluster_old(data, min_separation)
	n = size(data, 1);
	k = size(data, 2);
	...
	while 1
		...
		for i = 1:n
			...
			while high_idx < n && distance(old_cluster_data(high_idx + 1, 1), x_i) < min_separation
				high_idx = high_idx + 1;
			end
			while low_idx > 1 && distance(old_cluster_data(low_idx - 1, 1), x_i) < min_separation
				low_idx = low_idx - 1;
			end
		...
		if distance(cluster_data(1:n, 1), old_cluster_data(1:n, 1))/n < eps
			break;
		end
	end
	...
	for data_index = 1:n
		...
		if data_index == 1 || distance(data_cluster, current_cluster_mean) > eps
